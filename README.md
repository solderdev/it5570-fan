...
*Illustrative — assembled from the live register readings in "Verification results" below, not a real `sensors` capture. This fork hasn't been run end-to-end yet (see "LattePanda Sigma port" § Remaining work); replace this with actual output once the live driver test lands.*
...
|---|---|---|---|
| LattePanda Sigma | Intel Core i5-1340P (Alder Lake-P) | ITE EC-V14.6, `LP-EC-WTADLC1R210-V1.02` | Register map (reads and fan-control writes) verified live against raw EC ports; the compiled driver has not yet been run end-to-end — see "LattePanda Sigma port" § Remaining work |
...
Status: reads and fan control verified live, and the driver has since been adapted to this register map — register constants, PWM scaling (EC 0–100 % ↔ hwmon 0–255), and `pwm1_enable` mode mapping (0/1/2 → EC modes 3/1/2) are implemented and documented in `it5570_fan.c` and in "hwmon sysfs interface" below. Remaining work: the live driver test (loading the compiled module on hardware and confirming sysfs behavior end-to-end), then packaging.
